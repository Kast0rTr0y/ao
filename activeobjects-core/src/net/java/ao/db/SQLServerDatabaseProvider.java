...
public class SQLServerDatabaseProvider extends DatabaseProvider {
    ...
    protected List<String> renderAlterTableChangeColumn(DDLTable table, DDLField oldField, DDLField field)
    {
        ...
        if ((field.getDefaultValue() != null && !field.getDefaultValue().equals(oldField.getDefaultValue())) || (field.getDefaultValue() == null && oldField.getDefaultValue() != null))
        {
            sql.add(new StringBuilder()
                    .append("IF EXISTS (SELECT 1 FROM SYS.OBJECTS WHERE NAME = ").append(renderValue(defaultConstraintName(field))).append(") ")
                    .append("ALTER TABLE ").append(withSchema(table.getName()))
                    .append(" DROP CONSTRAINT ").append(defaultConstraintName(field))
                    .toString());
            
            sql.add(new StringBuilder()
                    .append("ALTER TABLE ").append(withSchema(table.getName()))
                    .append(" ADD CONSTRAINT ").append(defaultConstraintName(field))
    ...
    private String defaultConstraintName(DDLField field)
    {
        return "D_" + field.getName();
    }
    ...
    {
        return new StringBuilder().append(" CONSTRAINT ").append(defaultConstraintName(field)).append(" DEFAULT ").append(renderValue(field.getDefaultValue())).toString();
    }
	...
	protected String[] renderAlterTableAddColumn(DDLTable table, DDLField field) {
		...
		back.add("ALTER TABLE " + withSchema(table.getName()) + " ADD " + renderField(field, new RenderFieldOptions(true, true)));
